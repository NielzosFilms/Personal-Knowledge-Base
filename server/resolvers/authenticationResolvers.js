			...
			if (!user) {
				...
				return transporter
					.sendMail({
						...
						html: `<a href="https://${process.env.HOST}:${port}/create-user/token/${token}">Click here to create your account</a>`,
					})
					.then(async (res) => {
		...
		sendChangePasswordEmail: async (root, {email}, {models}) => {
			...
			return transporter
				.sendMail({
					...
					html: `<a href="https://${process.env.HOST}:${port}/reset/${user.id}/token/${token}">Click here to reset your password / change username</a>`,
				})
				.then(async (res) => {
					await models.EmailToken.destroy({
						where: {
